...
- **[avelikiy/great_cto](https://github.com/avelikiy/great_cto)** - Full SDLC pipeline as a Claude Code plugin: 7 specialised subagents (tech-lead, senior-dev, qa-engineer, security-officer, devops, l3-support, project-auditor), 16 commands, 11 archetypes, 13 compliance frameworks. Self-improving via `/crystallize`: incident learnings promote to a global pattern library that surfaces in every agent's Step 0 across all projects (verified MTTR ↓ 94% on second occurrence). MCP-native (Grafana monitoring, Kimi K2 LLM router). Files only, no SaaS. Install: `npx great-cto init`
- **[avelikiy/discovery-skill](https://github.com/avelikiy/great_cto/blob/main/skills/great_cto/references/discovery.md)** - Structured 8-question framework for project setup. When user input is sparse (`/start "ai stuff"`), asks audience/pain/compliance/team-size questions and proposes 2-3 concrete approaches with explicit tradeoffs. Option C is mandatory and is sometimes "don't build it - use a vendor." Tested across 24 archetype scenarios
- **[avelikiy/knowledge-extraction](https://github.com/avelikiy/great_cto/blob/main/skills/great_cto/references/knowledge-extraction.md)** - Schema and protocol for capturing post-incident learnings as structured YAML files. Privacy scan blocks project names, URLs, and credentials before save. Feeds the `/crystallize` global pattern library across all projects on the same machine
