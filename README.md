...
- **[adelaidasofia/humanizer](https://github.com/adelaidasofia/humanizer)** - Fork of blader/humanizer with v2.6+ enhancements: voice fingerprint mode (statistical fingerprint built from the author's own writing corpus, anchors rewrites to actual sentence rhythm and vocabulary), bilingual EN/ES rule library with Spanglish handling, AI-iness density check for adaptive pass strength, mandatory voice calibration, `--diff` mode that scores any draft 0-100 against the fingerprint
...
- **[adelaidasofia/ai-brain-starter](https://github.com/adelaidasofia/ai-brain-starter)** - Verification harness around an AI agent so memory compounds instead of corrupts. Markdown vault as ground truth, deterministic pre-write hooks, bi-temporal rule lineage, session-end cascade. Bundled skills: journaling with an advisory panel, weekly + monthly insight cycles, knowledge graph extraction with community detection, drift detection, rule-conflict detection, ingest-* connector family (Slack, WhatsApp, Notion, Linear, GitHub, Gmail, YouTube), creator-economy vertical pack. In production against a 10K-file vault for 8+ months. Inspired by Microsoft DELEGATE-52 finding that frontier LLMs corrupt 25% of professional content over 20 edits
