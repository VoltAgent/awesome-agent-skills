...
- **[kubestellar/console](https://github.com/kubestellar/console)** - Multi-cluster Kubernetes dashboard with AI-powered operations. The kc-agent MCP server bridges coding agents to kubeconfig and Kubernetes APIs for cluster management, policy enforcement, observability, and 20+ CNCF project integrations. Ships with 10+ built-in agent skills (@perf-test, @cache-test, @ci-status, @rca, @tdd, @k8s-debug). Supports Claude Code, Codex, Gemini CLI.
